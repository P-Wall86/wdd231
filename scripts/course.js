...
const creditsMessage = document.querySelector('.credits-message');

function calculateTotalCredits(filteredCourses) {
    return filteredCourses.reduce((total, course) => total + course.credits, 0);
}

function updateCreditsMessage(totalCredits) {
    creditsMessage.textContent = `Total credits required: ${totalCredits}`;
}
...
function filterCourses(filter) {
    ...
    updateCreditsMessage(totalCredits);

    filteredCourses.forEach(course => {
        const btn = document.createElement('button');
        btn.textContent = `${course.subject} ${course.number}: ${course.title}`;
